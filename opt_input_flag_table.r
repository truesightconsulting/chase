...
curve=merge(curve,ex.media[,c("chan2_id","chan1_id","flag_chan"),with=F],
            by=c("chan2_id","chan1_id"))
...
# ex.cstr=merge(ex.cstr,ex.media[,c("chan2_id","chan1_id","flag_chan"),with=F],
#               by=c("chan2_id","chan1_id"),all.y=T)
# ex.cstr=merge(ex.cstr,ex.dma[,c("dma1_id","flag_dma"),with=F],
#               by=c("dma1_id"),all.y=T)

ex.cstr.hidden=merge(ex.cstr.hidden,ex.media[,c("chan2_id","chan1_id","flag_chan"),with=F],
                     by=c("chan2_id","chan1_id"))
