...
if (ex.setup$optimization_type %in% c(3,4)){
  ...
  index=paste(ex.media$chan1_id,ex.media$chan2_id,sep="+") %in% 
    unique(paste(ex.plan.input$chan1_id,ex.plan.input$chan2_id,sep="+")) 
  ex.media$flag_chan[index]=1
  ex.media$flag_chan[!index]=0
  index=ex.dma$dma1_id %in% unique(ex.plan.input$dma1_id) 
